...
def makeToyTopoConfig(routers:List[str], switches:List[str], hosts:List[str], links:List[Tuple[str,str]],
    ...
    for rName in routers: mockRouterCfgs[rName] = RouterConfig(rName, MOCK_IP, MOCK_INTF)
    for sName in switches: mockSwitchCfgs[sName] = SwitchConfig(sName)
    for hName in hosts: mockHostCfgs[hName] = HostConfig(hName, MOCK_IP, MOCK_DEFLT_ROUTE)
