...
def parseXML(data: str) -> ToyTopoConfig:
    ...
    if XMLrouterList is None: raise XMLParseError('No routerList specified', data)

    XMLswitchList:ElementTree.ElementTree = XMLconfigurations.find('switchList')
    if XMLswitchList is None: raise XMLParseError('No switchList specified', data)

    XMLhostList:ElementTree.ElementTree = XMLconfigurations.find('hostList')
    if XMLhostList is None: raise XMLParseError('No hostList specified', data)

    XMLlinkList:ElementTree.ElementTree = XMLconfigurations.find('linkList')
    if XMLlinkList is None: raise XMLParseError('No linkList specified', data)
